...
function buildAgent(learningRate)
  local modelMean2 = nn.Sequential():add(nn.Linear(15, 4))
  local modelStdev2 = nn.Sequential():add(nn.Linear(15, 4)):add(nn.Exp())
  ...
  local policy = rl.GaussianPolicy(4)
...
function step(iterationsLimit, trajectoriesLimit)
  ...
  local action1, action2, action3, action4 = actions[1][1], actions[1][2], actions[1][3], actions[1][4]


  local r, sprime, t = sim:step({{action1}, {action2}, {action3}, {action4}}) -- take a step for four lions
...
function main()
  ...
  writedata("singleAgentReinforceTest.out")
